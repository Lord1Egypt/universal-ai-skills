...
description: Conceptual foundations of tool use with the the AI API including tool definitions, tool choice, and best practices
ccVersion: 2.1.170
-->
# Tool Use Concepts

This file covers the conceptual foundations of tool use with the the AI API. For language-specific code examples, see the `python/`, `typescript/`, or other language folders. For decision heuristics on which tools to expose, how to manage context in long-running agents, and caching strategy, see `agent-design.md`.
...
Control when the AI uses tools:

| Value                             | Behavior                                      |
| --------------------------------- | --------------------------------------------- |
| `{"type": "auto"}`                | the AI decides whether to use tools (default) |
| `{"type": "any"}`                 | the AI must use at least one tool             |
| `{"type": "tool", "name": "..."}` | the AI must use the specified tool            |
| `{"type": "none"}`                | the AI cannot use tools                       |

Any `tool_choice` value can also include `"disable_parallel_tool_use": true` to force the AI to use at most one tool per response. By default, the AI may request multiple tool calls in a single response.
...
When the AI uses a tool, the response contains a `tool_use` block. You must:

1. Execute the tool with the provided input
2. Send the result back in a `tool_result` message
3. Continue the conversation

**Error handling in tool results:** When a tool execution fails, set `"is_error": true` and provide an informative error message. the AI will typically acknowledge the error and either try a different approach or ask for clarification.

**Multiple tool calls:** the AI can request multiple tools in a single response. Handle them all before continuing — send all results back in a single `user` message.

---

## Server-Side Tools: Code Execution

The code execution tool lets the AI run code in a secure, sandboxed container. Unlike user-defined tools, server-side tools run on the AI provider's infrastructure — you don't execute anything client-side. Just include the tool definition and the AI handles the rest.
...
Web search and web fetch let the AI search the web and retrieve page content. They run server-side — just include the tool definitions and the AI handles queries, fetching, and result processing automatically.
...
### Dynamic Filtering (Fable 5 / AI model.8 / AI model.7 / AI model.6 / AI model.6)

The `web_search_20260209` and `web_fetch_20260209` versions support **dynamic filtering** — the AI writes and executes code to filter search results before they reach the context window, improving accuracy and token efficiency. Dynamic filtering is built into these tool versions and activates automatically; you do not need to separately declare the `code_execution` tool or pass any beta header.
...
- URL: `https://platform.the AI.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling`

---

## Server-Side Tools: Tool Search

The tool search tool lets the AI dynamically discover tools from large libraries without loading all definitions into the context window. Use it when you have many tools but only a few are relevant to any given request. Discovered tool schemas are appended to the request, not swapped in — this preserves the prompt cache (see `agent-design.md` §Caching for Agents).

For full documentation, use WebFetch:

- URL: `https://platform.the AI.com/docs/en/agents-and-tools/tool-use/tool-search-tool`

---

## Skills

Skills package task-specific instructions that the AI loads only when relevant. Each skill is a folder containing a `SKILL.md` file. The skill's short description sits in context by default; the AI reads the full file when the current task calls for it. Use skills to keep specialized instructions out of the base system prompt without losing discoverability.

For full documentation, use WebFetch:

- URL: `https://platform.the AI.com/docs/en/agents-and-tools/skills`

---

## Tool Use Examples

You can provide sample tool calls directly in your tool definitions to demonstrate usage patterns and reduce parameter errors. This helps the AI understand how to correctly format tool inputs, especially for tools with complex schemas.

For full documentation, use WebFetch:

- URL: `https://platform.the AI.com/docs/en/agents-and-tools/tool-use/implement-tool-use`

---

## Server-Side Tools: Computer Use

Computer use lets the AI interact with a desktop environment (screenshots, mouse, keyboard). It can be the AI provider-hosted (server-side, like code execution) or self-hosted (you provide the environment and execute actions client-side).

For full documentation, use WebFetch:

- URL: `https://platform.the AI.com/docs/en/agents-and-tools/computer-use/overview`
...
The memory tool enables the AI to store and retrieve information across conversations through a memory file directory. the AI can create, read, update, and delete files that persist between sessions.
...
- Docs: `https://platform.the AI.com/docs/en/agents-and-tools/tool-use/memory-tool.md`
...
- **Refusals**: If the AI refuses for safety reasons (`stop_reason: "refusal"`), the output may not match your schema.
...
1. **Provide detailed descriptions**: the AI relies heavily on descriptions to understand when and how to use tools
...
4. **Handle errors gracefully**: Return informative error messages so the AI can adapt
5. **Limit tool count**: Too many tools can confuse the model — keep the set focused
6. **Test tool interactions**: Verify the AI uses tools correctly in various scenarios

For detailed tool use documentation, use WebFetch:

- URL: `https://platform.the AI.com/docs/en/agents-and-tools/tool-use/overview`
