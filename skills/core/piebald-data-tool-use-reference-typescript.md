...
```typescript
import the AI provider from "@the AI provider-ai/sdk";
import { betaZodTool } from "@the AI provider-ai/sdk/helpers/beta/zod";
import { z } from "zod";
...
- Iteration stops automatically when the AI has no more tool calls
...
const client = new the AI provider();
const tools: the AI provider.Tool[] = [...]; // Your tool definitions
let messages: the AI provider.MessageParam[] = [{ role: "user", content: userInput }];
...
while (true) {
  ...
  const toolUseBlocks = response.content.filter(
    (b): b is the AI provider.ToolUseBlock => b.type === "tool_use",
  );

  messages.push({ role: "assistant", content: response.content });

  const toolResults: the AI provider.ToolResultBlockParam[] = [];
...
const client = new the AI provider();
const tools: the AI provider.Tool[] = [...];
let messages: the AI provider.MessageParam[] = [{ role: "user", content: userInput }];
...
> **Error handling in the loop:** Use the SDK's typed exceptions (e.g., `the AI provider.RateLimitError`, `the AI provider.APIError`) — see [Error Handling](./README.md#error-handling) for examples. Don't check error messages with string matching.

> **SDK types:** Use `the AI provider.MessageParam`, `the AI provider.Tool`, `the AI provider.ToolUseBlock`, `the AI provider.ToolResultBlockParam`, `the AI provider.Message`, etc. for all API-related data structures. Don't redefine equivalent interfaces.
...
**Don't type-annotate as `Tool[]`** — `Tool` is just the custom-tool variant. Let structural typing infer from the `tools` param, or annotate as `the AI provider.Messages.ToolUnion[]` if you must:
...
// const tools: the AI provider.Tool[] = [{ type: "text_editor_20250728", ... }]
```
...
```typescript
import the AI provider, { toFile } from "@the AI provider-ai/sdk";
...
const response = await client.messages.create(
  ...
  { headers: { "the AI provider-beta": "files-api-2025-04-14" } },
);
```
...
} from "@the AI provider-ai/sdk/helpers/beta/memory";
...
- `https://github.com/the AI providers/the AI provider-sdk-typescript/blob/main/examples/tools-helpers-memory.ts`
...
import the AI provider from "@the AI provider-ai/sdk";
import { z } from "zod";
import { zodOutputFormat } from "@the AI provider-ai/sdk/helpers/zod";
