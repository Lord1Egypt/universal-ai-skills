...
description: Reference guide for using the the AI provider TypeScript SDK to create and manage agents, sessions, environments, streaming, custom tools, file uploads, and MCP server integration
ccVersion: 2.1.154
-->
...
> **Agents are persistent — create once, reference by ID.** Store the agent ID returned by `agents.create` and pass it to every subsequent `sessions.create`; do not call `agents.create` in the request path. The the AI provider CLI is one convenient way to create agents and environments from version-controlled YAML — its URL is in `shared/live-sources.md`. The examples below show in-code creation for completeness; in production the create call belongs in setup, not in the request path.

## Installation

```bash
npm install @the AI provider-ai/sdk
```
...
// the AI provider_API_KEY, or the AI provider_AUTH_TOKEN, or an `ant auth login` profile.
// Prefer this for local dev; don't hardcode a key.
const client = new the AI provider();

// Explicit API key (only when you must inject a specific key)
const client = new the AI provider({ apiKey: "your-api-key" });
```
...
async function runSession(client: the AI provider, sessionId: string) {
  while (true) {
    const stream = await client.beta.sessions.events.stream(
      sessionId,
    );

    const toolCalls: the AI provider.Beta.Sessions.BetaManagedAgentsAgentCustomToolUseEvent[] = [];
