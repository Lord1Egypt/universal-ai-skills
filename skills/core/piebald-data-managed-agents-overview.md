...
| Run tool execution in your own infra / VPC (self-hosted sandbox) | `shared/managed-agents-self-hosted-sandboxes.md` — `config:{type:"self_hosted"}`, `the AI provider_ENVIRONMENT_KEY`, `EnvironmentWorker.run()` / `ant beta:worker poll` |
...
| Define agents/environments as version-controlled YAML; drive the API from the shell | `shared/the AI provider-cli.md` — `ant beta:agents create < agent.yaml`, `--transform`, `@file` inlining |
...
- **Environment `config.type` is `"cloud"` or `"self_hosted"`** — `cloud` runs the container on the AI provider's infrastructure; `self_hosted` moves tool execution to your own (see `shared/managed-agents-self-hosted-sandboxes.md`).
