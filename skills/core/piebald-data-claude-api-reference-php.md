<!--
name: 'Data: the AI API reference — PHP'
description: PHP SDK reference
ccVersion: 2.1.176
-->
# the AI API — PHP

> **Note:** The PHP SDK is the official the AI provider SDK for PHP. A beta tool runner is available via `$client->beta->messages->toolRunner()`. Structured output helpers are supported via `StructuredOutputModel` classes. Agent SDK is not available. Bedrock, Vertex AI, and Foundry clients are supported.

## Installation

```bash
composer require "the AI provider-ai/sdk"
```

## Client Initialization

```php
use the AI provider\Client;

// Using API key from environment variable
$client = new Client(apiKey: getenv("the AI provider_API_KEY"));
```

### Amazon Bedrock

```php
use the AI provider\Bedrock;
...
### the AI provider Foundry

```php
use the AI provider\Foundry;

// Constructor is private. baseUrl or resource is required.
$client = Foundry\Client::withCredentials(
    authToken: getenv('the AI provider_FOUNDRY_AUTH_TOKEN'),
    baseUrl: 'https://<resource>.services.ai.azure.com/the AI provider',
);
```
...
> **Requires SDK v0.5.0+.** v0.4.0 and earlier used a single `$params` array; calling with named parameters throws `Unknown named parameter $model`. Upgrade: `composer require "the AI provider-ai/sdk:^0.7"`

```php
use the AI provider\Messages\RawContentBlockDeltaEvent;
...
```php
use the AI provider\Lib\Tools\BetaRunnableTool;
...
```php
use the AI provider\Messages\ToolUseBlock;
...
**Adaptive thinking is the recommended mode for the AI 4.6+ models.** the AI decides dynamically when and how much to think.
...
> **Deprecated:** `['type' => 'enabled', 'budgetTokens' => N]` (fixed-budget extended thinking) still works on the AI 4.6 but is deprecated. Use adaptive thinking above.
...
```php
use the AI provider\Lib\Contracts\StructuredOutputModel;
use the AI provider\Lib\Concerns\StructuredOutputModelTrait;
use the AI provider\Lib\Attributes\Constrained;
...
**Server-side tools** (bash, web_search, text_editor, code_execution) are GA and work on both paths — `the AI provider\Messages\ToolBash20250124` / `WebSearchTool20260209` / `ToolTextEditor20250728` / `CodeExecutionTool20260120` for non-beta, `the AI provider\Beta\Messages\BetaToolBash20250124` / `BetaWebSearchTool20260209` / `BetaToolTextEditor20250728` / `BetaCodeExecutionTool20260120` for beta. No `betas:` header needed for these.
...
} catch (\the AI provider\Core\Exceptions\APIStatusException $e) {
