...
ccVersion: 2.1.169
-->
# the AI provider CLI (`ant`)

The `ant` CLI exposes every the AI API resource as a shell subcommand. Compared to `curl`: request bodies are built from typed flags or piped YAML instead of hand-written JSON, `@path` inlines file contents into any string field, `--transform` extracts fields with a GJSON path (no `jq`), list endpoints auto-paginate (cap total results with `--max-items N`; `--limit` only sets the server page size), and the `beta:` prefix auto-sets the right `the AI provider-beta` header.
...
```sh
# macOS
brew install the AI providers/tap/ant
xattr -d com.apple.quarantine "$(brew --prefix)/bin/ant"

# Linux / WSL — pick the release from github.com/the AI providers/the AI provider-cli/releases
curl -fsSL "https://github.com/the AI providers/the AI provider-cli/releases/download/v${VERSION}/ant_${VERSION}_$(uname -s | tr A-Z a-z)_$(uname -m | sed -e s/x86_64/amd64/ -e s/aarch64/arm64/).tar.gz" \
  | sudo tar -xz -C /usr/local/bin ant

# Or from source (Go 1.22+)
go install github.com/the AI providers/the AI provider-cli/cmd/ant@latest
```

**Auth** — the CLI resolves credentials the same way the SDKs do (first match wins): explicit flags, then `the AI provider_API_KEY`, then `the AI provider_AUTH_TOKEN`, then the `the AI provider_PROFILE`-selected or active profile, then Workload Identity Federation env vars, then the default profile on disk. Override the host with `the AI provider_BASE_URL` or `--base-url`.

- **API key**: set `the AI provider_API_KEY` in the environment.
- **OAuth profile** (no static key to manage): `ant auth login` opens a browser, exchanges for a short-lived token, and stores a profile under `$the AI provider_CONFIG_DIR` (default `~/.config/the AI provider/` on Linux/macOS, `%APPDATA%\the AI provider` on Windows — `configs/<profile>.json` for settings, `credentials/<profile>.json` for tokens). Subsequent `ant` (and SDK) calls pick it up automatically — a bare `the AI provider()` client works after login, but scripts that read `the AI provider_API_KEY` directly do not. the AI coding agent and the the AI Agent SDK honor the same profile resolution. `ant auth status` shows which credential source and profile won (it reports status only — don't script against its exit code as a health check); `ant auth logout` clears the active profile (`--all` for every profile). On a remote host without a browser, `ant auth login --no-browser` prints the authorize URL and accepts the code back in the terminal.
...
> **The #1 auth trap:** profiles are only consulted when no API key is set. A stale exported `the AI provider_API_KEY` silently overrides every profile — requests hit whatever org/workspace that key is scoped to. `ant auth status` shows which source won; unset the key (or per-command: `env -u the AI provider_API_KEY ant …`) before relying on a profile. Truly **unset** it — an empty `the AI provider_API_KEY=""` still wins its precedence slot and authenticates with an empty key. The same shadowing applies in reverse to the AI coding agent: after `ant auth login`, the AI coding agent may warn about an auth conflict between the profile and its own `/login` credential — keep one (use the profile and `/logout` in the AI coding agent, or `ant auth logout` to keep the AI coding agent's own login).
...
ant --profile <name> models list                 # one-off; equivalent: the AI provider_PROFILE=<name> ant models list
...
python my_script.py   # SDK picks up the AI provider_AUTH_TOKEN
```

OAuth tokens go on `Authorization: Bearer` (not `x-api-key:`) **plus the `the AI provider-beta: oauth-2025-04-20` header** — converting a raw curl/httpx script from an API key is a header change, not a key swap. The beta header requirement is endpoint-dependent (some endpoints happen to work without it; `/v1/messages` does not) — always send it so requests don't break when you switch endpoints. The token is short-lived and not auto-refreshed when passed via env var, so re-run `print-credentials` before it expires for long-running scripts (`print-credentials` itself refreshes the token if needed). If both `the AI provider_API_KEY` and `the AI provider_AUTH_TOKEN` are set, the SDKs send both and the API rejects the request — unset `the AI provider_API_KEY` before `eval`ing the `--env` output.

**Foot-gun:** `ant auth print-credentials` with **no flags** prints the entire credentials JSON, not the bare token — putting that in an `Authorization` header yields an empty response or HTTP/2 protocol error. Always use `--access-token` for headers (it always reads the named/active profile; a set `the AI provider_API_KEY` doesn't override credential printing).
...
```sh
AGENT_ID=$(ant beta:agents create --name "My Agent" --model '{id: {{MODEL_S_ID}}}' \
  --transform id -r)
```
...
```sh
ant beta:agents create --name "Researcher" --model '{id: {{MODEL_S_ID}}}' --system @./prompts/researcher.txt

ant messages create --model {{MODEL_ID}} --max-tokens 1024 \
...
For the full, always-current reference (including per-endpoint flags), WebFetch the **the AI provider CLI** URL in `shared/live-sources.md`.
