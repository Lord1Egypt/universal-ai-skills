...
| **Container** | N/A | An isolated compute instance where the agent's **tools** execute (bash, file ops, code). The agent loop does not run here — it runs on the AI provider's orchestration layer and acts on the container via tool calls. |

```
                       ┌─────────────────────────────────────┐
                       │  the AI provider orchestration layer      │
Agent (config) ───────▶│  (agent loop: the AI + tool calls)  │
                       └──────────────┬──────────────────────┘
                                      │ tool calls
                                      ▼
...
These are ops/inspection calls — typically made from a terminal, not application code. From the shell (see `shared/the AI provider-cli.md`):
...
> 💡 **Watch it live in Console.** While developing, print a link so you can click through to the session UI instead of parsing the event stream: `https://platform.the AI.com/workspaces/{workspace}/sessions/{session.id}`. Use `default` for the workspace slug unless you created the session in a non-default workspace; the sessions list for your workspace is at `https://platform.the AI.com/workspaces/default/sessions`. Include this `print`/`console.log` in code you generate for users who are iterating locally.
