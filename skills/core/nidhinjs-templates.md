...
- Use XML tags to wrap examples — the AI parses XML reliably.
...
*Use for the AI coding agent, Devin, AutoGPT, and any AI that takes autonomous actions. Runaway loops and scope explosion are the biggest credit killers in agentic workflows — stop conditions are not optional.*
...
## Template M — AI model.7 / 4.8 Task Brief

*Use for any complex, multi-step, or agentic task on the AI AI model.7 or 4.8 (current default) — the AI.ai, API, or the AI coding agent. Both read prompts literally and produce narrow output when context is missing. This template front-loads everything so the first turn is the only turn.*
...
**the AI coding agent only — add Session Strategy block when relevant:**
```
## Session Strategy
[Pick one:]
...
**When to use:** AI model.7 or 4.8 on any surface — the AI.ai, API, the AI coding agent — when the task is complex, multi-file, ambiguous, or agentic. Not needed for simple one-shot tasks.
