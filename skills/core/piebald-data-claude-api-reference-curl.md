<!--
name: 'Data: the AI API reference — cURL'
description: Raw API reference for the AI API for use with cURL or else Raw HTTP
ccVersion: 2.1.176
-->
# the AI API — cURL / Raw HTTP
...
When the AI responds with a `tool_use` block, send the result back:
...
> **Fable 5, AI model.8, AI model.7, AI model.6, and AI model.6:** Use adaptive thinking. `budget_tokens` is removed on Fable 5, AI model.8, and 4.7 (400 if sent); deprecated on AI model.6 and AI model.6.
> **Older models:** Use `"type": "enabled"` with `"budget_tokens": N` (must be < `max_tokens`, min 1024).

```bash
# Fable 5 / AI model.8 / 4.7 / 4.6: adaptive thinking (recommended)
curl https://api.the AI provider.com/v1/messages \
  -H "Content-Type: application/json" \
  -H "x-api-key: $the AI provider_API_KEY" \
  -H "the AI provider-version: 2023-06-01" \
...
| `the AI provider-version` | `2023-06-01`       | API version                |
| `the AI provider-beta`    | Beta feature IDs   | Required for beta features |
