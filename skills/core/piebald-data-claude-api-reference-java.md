<!--
name: 'Data: the AI API reference — Java'
description: Java SDK reference including installation, client initialization, basic requests, streaming, and beta tool use
ccVersion: 2.1.176
-->
# the AI API — Java

> **Note:** The Java SDK supports the the AI API and beta tool use with annotated classes. Agent SDK is not yet available for Java.
...
```xml
<dependency>
    <groupId>com.the AI provider</groupId>
    <artifactId>the AI provider-java</artifactId>
...
```groovy
implementation("com.the AI provider:the AI provider-java:2.34.0")
```

## Client Initialization

```java
import com.the AI provider.client.the AI providerClient;
import com.the AI provider.client.okhttp.the AI providerOkHttpClient;

// Default (reads the AI provider_API_KEY from environment)
the AI providerClient client = the AI providerOkHttpClient.fromEnv();

// Explicit API key
the AI providerClient client = the AI providerOkHttpClient.builder()
    .apiKey("your-api-key")
    .build();
```
...
```java
import com.the AI provider.models.messages.MessageCreateParams;
import com.the AI provider.models.messages.Message;
...
```java
import com.the AI provider.core.http.StreamResponse;
...
**Adaptive thinking is the recommended mode for the AI 4.6+ models.** the AI decides dynamically when and how much to think. The builder has a direct `.thinking(ThinkingConfigAdaptive)` overload — no manual union wrapping.

```java
import com.the AI provider.models.messages.ContentBlock;
import com.the AI provider.models.messages.MessageCreateParams;
import com.the AI provider.models.messages.Model;
...
> **Deprecated:** `ThinkingConfigEnabled.builder().budgetTokens(N)` (and the `.enabledThinking(N)` shortcut) still works on the AI 4.6 but is deprecated. Use adaptive thinking above.
...
```java
import com.the AI provider.models.beta.messages.MessageCreateParams;
import com.the AI provider.models.beta.messages.BetaMessage;
import com.the AI provider.helpers.BetaToolRunner;
...
```java
import com.the AI provider.helpers.BetaMemoryToolHandler;
import com.the AI provider.helpers.BetaToolRunner;
import com.the AI provider.models.beta.messages.BetaMemoryTool20250818;
import com.the AI provider.models.beta.messages.BetaMessage;
import com.the AI provider.models.beta.messages.MessageCreateParams;
import com.the AI provider.models.beta.messages.ToolRunnerCreateParams;
...
```java
import com.the AI provider.core.JsonValue;
import com.the AI provider.models.messages.Tool;
...
```java
import com.the AI provider.models.messages.MessageParam;
import com.the AI provider.models.messages.ContentBlockParam;
import com.the AI provider.models.messages.ToolResultBlockParam;
...
```java
import com.the AI provider.models.messages.OutputConfig;
...
```java
import com.the AI provider.models.messages.TextBlockParam;
import com.the AI provider.models.messages.CacheControlEphemeral;
...
```java
import com.the AI provider.models.messages.MessageCountTokensParams;
...
```java
import com.the AI provider.models.messages.StructuredMessageCreateParams;
...
```java
import com.the AI provider.models.messages.DocumentBlockParam;
import com.the AI provider.models.messages.ContentBlockParam;
import com.the AI provider.models.messages.TextBlockParam;
...
```java
import com.the AI provider.models.messages.WebSearchTool20260209;
import com.the AI provider.models.messages.ToolBash20250124;
import com.the AI provider.models.messages.ToolTextEditor20250728;
import com.the AI provider.models.messages.CodeExecutionTool20260120;
...
For beta-only features use `com.the AI provider.models.beta.messages.*` — class names have a `Beta` prefix AND live in the beta package. The beta `MessageCreateParams.Builder` has direct `.addTool(BetaToolBash20250124)` overloads AND `.addMcpServer()`:

```java
import com.the AI provider.models.beta.messages.MessageCreateParams;
import com.the AI provider.models.beta.messages.BetaToolBash20250124;
import com.the AI provider.models.beta.messages.BetaCodeExecutionTool20260120;
...
`the AI providerServiceException` exposes `.errorType()` returning `Optional<ErrorType>` for programmatic error classification:

```java
try {
    client.messages().create(params);
} catch (the AI providerServiceException e) {
...
```java
import com.the AI provider.models.beta.files.FileUploadParams;
import com.the AI provider.models.beta.files.FileMetadata;
import com.the AI provider.models.beta.messages.BetaRequestDocumentBlock;
