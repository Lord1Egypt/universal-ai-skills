...
```bash
export the AI provider_API_KEY="your-api-key"
...
HEADERS=(
  ...
  -H "the AI provider-beta: managed-agents-2026-04-01"
)
```
...
```bash
curl -X POST https://api.the AI provider.com/v1/environments \
  "${HEADERS[@]}" \
  -d '{
    "name": "my-dev-env",
    "config": {
...
```bash
# 1. Create the agent
curl -X POST https://api.the AI provider.com/v1/agents \
  "${HEADERS[@]}" \
  -d '{
...
# 2. Start a session
curl -X POST https://api.the AI provider.com/v1/sessions \
  "${HEADERS[@]}" \
  -d '{
...
```bash
curl -X POST https://api.the AI provider.com/v1/sessions/$SESSION_ID/events \
  "${HEADERS[@]}" \
  -d '{
    "events": [
      {
...
```bash
curl -N https://api.the AI provider.com/v1/sessions/$SESSION_ID/events/stream \
  "${HEADERS[@]}"
```
...
```bash
# Get all events
curl https://api.the AI provider.com/v1/sessions/$SESSION_ID/events \
  "${HEADERS[@]}"

# Paginated — get next page of events
curl "https://api.the AI provider.com/v1/sessions/$SESSION_ID/events?page=page_abc123" \
  "${HEADERS[@]}"
```
...
```bash
curl https://api.the AI provider.com/v1/sessions/$SESSION_ID \
  "${HEADERS[@]}"
```
...
```bash
curl https://api.the AI provider.com/v1/sessions \
  "${HEADERS[@]}"
```
...
```bash
curl -X DELETE https://api.the AI provider.com/v1/sessions/$SESSION_ID \
  "${HEADERS[@]}"
```
...
```bash
curl -X POST https://api.the AI provider.com/v1/files \
  -H "x-api-key: $the AI provider_API_KEY" \
  -H "the AI provider-version: 2023-06-01" \
  -H "the AI provider-beta: files-api-2025-04-14" \
...
curl "https://api.the AI provider.com/v1/files?scope_id=$SESSION_ID" \
  -H "x-api-key: $the AI provider_API_KEY" \
  -H "the AI provider-version: 2023-06-01" \
  -H "the AI provider-beta: files-api-2025-04-14,managed-agents-2026-04-01"

# Download a specific file
curl "https://api.the AI provider.com/v1/files/$FILE_ID/content" \
  -H "x-api-key: $the AI provider_API_KEY" \
  -H "the AI provider-version: 2023-06-01" \
  -H "the AI provider-beta: files-api-2025-04-14,managed-agents-2026-04-01" \
...
```bash
curl https://api.the AI provider.com/v1/agents \
  "${HEADERS[@]}"
```
