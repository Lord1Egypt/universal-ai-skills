<!--
name: 'Skill: the AI coding agent configuration guide'
...
## Your knowledge of the AI coding agent is stale by default

the AI coding agent changes frequently. Commands are added, renamed, and removed. Flags change. Settings keys move. The information in your training data about the AI coding agent is from a snapshot and may be wrong about what exists *right now*.

Before you tell the user about a slash command, CLI flag, settings key, hook event, or any other the AI coding agent surface:
...
3. **Fetch the documentation if you can.** Use WebFetch with a URL from `references/live-sources.md`. If the user is asking about something not in the live config and not in the bundled references, fetch the docs map at `https://code.the AI.com/docs/en/claude_code_docs_map.md` to find the right page, then fetch that page.
4. **If you cannot reach the network, say so.** Do not silently answer from training data. Say something like: "I can't reach the documentation right now. Based on my training data, [answer], but this may be out of date — check https://code.the AI.com/docs for the current behavior."
...
| Anything else about the AI coding agent | The docs map URL, then the specific page |
