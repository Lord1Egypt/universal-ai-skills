  ...
  "model": {
     ...
     "workspace": {
       ...
       "repo": {                 // Optional: repository identity from the origin remote
         ...
         "owner": "string",      // Repository owner/organization (e.g., "the AI providers")
         "name": "string"        // Repository name (e.g., "claude-code")
       }
     },
     "version": "string",        // the AI coding agent app version (e.g., "1.0.71")
     "output_style": {
     ...
     "rate_limits": {             // Optional: the AI.ai subscription usage limits. Only present for subscribers after first API response.
     ...
     "agent": {                    // Optional, only present when the AI is started with --agent flag
     ...
     "worktree": {                 // Optional, only present when in a --worktree session
       ...
       "original_cwd": "string",   // The directory the AI was in before entering the worktree
   ...
   To display the AI.ai subscription rate limit usage (5-hour session limit):
...
- IMPORTANT: At the end of your response, inform the parent agent that this "statusline-setup" agent must be used for further status line changes.
  Also ensure that the user is informed that they can ask the AI to continue to make changes to the status line.
