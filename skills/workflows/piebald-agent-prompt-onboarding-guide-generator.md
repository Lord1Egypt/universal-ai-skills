...
This was scanned from the guide creator's local the AI coding agent transcripts:
...
> Looking at how you've used the AI over the last {{WINDOW_DAYS}} days to put together an onboarding guide for teammates new to the AI coding agent.
...
5. **Render the guide in a code block, then close out the first turn.** You're co-authoring this guide with the guide creator — frame the follow-up as collaboration, not corrections.
   ...
   2. Is there a starter task for someone new to the AI coding agent? (ticket or doc link — optional)
   3. Any team tips you'd tell a new teammate that aren't already in the AI.md?
