...
**User**: "Use the AI coding agent to explain this codebase structure without changing anything."
...
- No `--model`, so the AI coding agent uses the configured default model.
- `--tools` keeps the session read-only and predictable.

## Safe Bug Fix

**User**: "Use the AI coding agent headless mode to fix the authentication bug and run the relevant tests."
...
- `--append-system-prompt` keeps the AI coding agent defaults intact.
...
**User**: "Pick up where the AI coding agent left off."
...
**User**: "Use the AI coding agent plus my monitoring MCP servers to investigate payment API latency."
...
**User**: "Run the AI coding agent without permission prompts inside my CI container."
...
**User**: "Give me a read-only the AI coding agent command that counts lines of code by language."
